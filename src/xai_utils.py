...
def layer_finder(k_model, model_arch, pool_input=True):
    ...
    if model_arch == "VGG":
        ...
        for i in range(len(k_model.layers)):
            ...
            if (
                k_model.layers[i + 1].output.get_shape()[2]
                < k_model.layers[i].output.get_shape()[2] - 4
            ):
                if pool_input == True:
                    if type(k_model.layers[i]) == tf.keras.layers.InputLayer:
                        continue
                    if "ZeroPadding2D" in str(type(k_model.layers[i])):
                        if type(k_model.layers[i - 1]) == tf.keras.layers.InputLayer:
                            continue
...
def create_attribution_masks(
    img, model, layers, class_index, max_mask_num, interp="bilinear"
):
    """
    ...
    for i in range(len(layers)):
        ...
        if interp == "bilinear":
            ...
            raise ValueError("You have selected an unsupported interpolation type.")
...
def visualize_layers(img, model, class_index, masks, H=224, W=224, C=3, batch_size=128):
    ...
    if masks.shape[2] <= batch_size:
        preds_masked = np.append(
            preds_masked, model(X, training=False)[:, class_index], axis=0
        )
    else:
        for i in range(0, masks.shape[2] - batch_size, batch_size):
            preds_masked = np.append(
                preds_masked,
                model(X[i : i + batch_size], training=False)[:, class_index],
                axis=0,
            )
        preds_masked = np.append(
            preds_masked,
            model(X[i + batch_size :], training=False)[:, class_index],
            axis=0,
        )
...
def otsu(I, nbins=256, tau=1.5):
    ...
    # print(t)
    k = round(t * tau)
    if np.sum(hist[int(k) : 256]) < 0.1 * np.sum(hist):
        # print('happened')
        return t * tau / nbins
    else:
        return t / nbins
...
def otsu_sigmoid(I, nbins=256, T=100.0, tau=1.5):
    """
...
def fuse_visualization_maps(exmaps, fusion_type="otsu", T=100.0):
    """
...
def SISE(
    ...
    """
    masks = create_attribution_masks(
        img,
        model,
        layers,
...
def weighted_fusion(w, exmaps, T=100.0):
    """
    ...
    e234 = np.multiply(
        (e23 * w_post[1] + exmaps[:, :, 2] * (2 - w_post[1])),
        otsu_sigmoid(exmaps[:, :, 2], T=T),
    )
    e2345 = np.multiply(
        (e234 * w_post[2] + exmaps[:, :, 3] * (2 - w_post[2])),
